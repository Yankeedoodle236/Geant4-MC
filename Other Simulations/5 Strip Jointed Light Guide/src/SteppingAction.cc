...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  auto detVolume_left = detConstruction->GetLeftVolume();
  auto detVolume_right = detConstruction->GetRightVolume();
  auto detVolume_mid = detConstruction->GetMidVolume();
  auto scint_volume = detConstruction->GetScintVolume();
  G4bool condition2 = step->IsFirstStepInVolume();
  ...
  if(PreVolume != detVolume && PostVolume == detVolume && condition2 == true)
  {
    counter_step += 1;
  ...
  G4int counter_step2 = 0;
  if(PreVolume == scint_volume && PostVolume == detVolume_left)
  {
    counter_step2 += 1;
  }
  fEventAction->AddCount_left(counter_step2);

  G4int counter_step3 = 0;
  if(PreVolume == scint_volume && PostVolume == detVolume_right)
  {
    counter_step3 += 1;
  }
  fEventAction->AddCount_right(counter_step3);
